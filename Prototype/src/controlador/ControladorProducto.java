...
public class ControladorProducto {
    ...
    public void ClonarProductoSeleccionado(JTable tablaProductos) {
        int fila = tablaProductos.getSelectedRow();
        if (fila >= 0) {
            // Obtener producto original
            ModeloProducto original = new ModeloProducto();
            original.setIdProducto(Integer.parseInt(tablaProductos.getValueAt(fila, 0).toString()));
            original.setNombreProducto(tablaProductos.getValueAt(fila, 1).toString());
            original.setPrecioProducto(Double.parseDouble(tablaProductos.getValueAt(fila, 2).toString()));
            original.setStockProducto(Integer.parseInt(tablaProductos.getValueAt(fila, 3).toString()));

            // Clonar
            ModeloProducto clon = original.clonar();

            // Modificar el nombre para diferenciar el clon
            clon.setNombreProducto(clon.getNombreProducto() + " (Copia)");
            clon.setIdProducto(0); 

            // Insertar el clon en la base de datos
            try {
                configuracion.Conexion objetoConexion = new configuracion.Conexion();
                String consulta = "INSERT INTO producto (nombre, precioProducto, stock) VALUES (?, ?, ?);";
                CallableStatement cs = objetoConexion.estableceConexion().prepareCall(consulta);
                cs.setString(1, clon.getNombreProducto());
                cs.setDouble(2, clon.getPrecioProducto());
                cs.setInt(3, clon.getStockProducto());
                cs.execute();
                JOptionPane.showMessageDialog(null, "Producto clonado exitosamente");
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Error al clonar producto: " + e.toString());
            }
        } else {
            JOptionPane.showMessageDialog(null, "Seleccione un producto para clonar");
        }
    }
