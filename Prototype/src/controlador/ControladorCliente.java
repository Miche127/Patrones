...
public class ControladorCliente {
    ...
    public void ClonarClienteSeleccionado(JTable tablaClientes) {
        int fila = tablaClientes.getSelectedRow();
        if (fila >= 0) {
            ModeloCliente original = new ModeloCliente();
            original.setIdCliente(Integer.parseInt(tablaClientes.getValueAt(fila, 0).toString()));
            original.setNombres(tablaClientes.getValueAt(fila, 1).toString());
            original.setApPaterno(tablaClientes.getValueAt(fila, 2).toString());
            original.setApMaterno(tablaClientes.getValueAt(fila, 3).toString());

            ModeloCliente clon = original.clonar();
            clon.setNombres(clon.getNombres() + " (Copia)");

            try {
                configuracion.Conexion objetoConexion = new configuracion.Conexion();
                String consulta = "INSERT INTO cliente (nombres, appaterno, apmaterno) VALUES (?, ?, ?);";
                CallableStatement cs = objetoConexion.estableceConexion().prepareCall(consulta);
                cs.setString(1, clon.getNombres());
                cs.setString(2, clon.getApPaterno());
                cs.setString(3, clon.getApMaterno());
                cs.execute();
                JOptionPane.showMessageDialog(null, "Cliente clonado exitosamente");
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Error al clonar cliente: " + e.toString());
            }
        } else {
            JOptionPane.showMessageDialog(null, "Seleccione un cliente para clonar");
        }
    }
